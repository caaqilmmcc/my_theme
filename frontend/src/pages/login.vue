...
const password = ref('mmcccc.')
const rememberMe = ref(false)
const CurrentUser = async (email) => {
	const res = await axios.get('/api/resource/User', {
		params: {
			filters: JSON.stringify({ username: email }),
...
const login = () => {
	axios({
		...
		.then(async (r) => {
			if (r.data) {
				const getUser = await CurrentUser(email.value)
				const UserReForm = {
					id: getUser[0].name,
					fullName: getUser[0].full_name,
					username: getUser[0].username,
					avatar: getUser[0].user_image,
				...
				localStorage.setItem('userData', JSON.stringify(UserReForm))
				localStorage.setItem(
					'accessToken',
					JSON.stringify('680e237cdc355e4.4c9f82915c34ffd')
				)
